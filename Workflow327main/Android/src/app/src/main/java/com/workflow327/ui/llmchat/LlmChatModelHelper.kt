...
import com.workflow327.common.cleanUpMediapipeTaskErrorMessage
import com.workflow327.data.Accelerator
import com.workflow327.data.ConfigKey
import com.workflow327.data.DEFAULT_MAX_TOKEN
import com.workflow327.data.DEFAULT_TEMPERATURE
import com.workflow327.data.DEFAULT_TOPK
import com.workflow327.data.DEFAULT_TOPP
import com.workflow327.data.MAX_IMAGE_COUNT
...
object LlmChatModelHelper {
  ...
  fun initialize(context: Context, model: Model, onDone: (String) -> Unit) {
    ...
    Log.d(TAG, "Initializing model '${model.name}' at path: $modelPath")
    Log.d(TAG, "Model details - imported: ${model.imported}, downloadFileName: ${model.downloadFileName}")
    
    // Check if model file exists
    val modelFile = java.io.File(modelPath)
    if (!modelFile.exists()) {
      Log.e(TAG, "Model file does not exist at path: $modelPath")
      Log.e(TAG, "Model file check - exists: ${modelFile.exists()}, canRead: ${modelFile.canRead()}, length: ${modelFile.length()}")
      onDone("Model file not found at path: $modelPath")
      return
    }
    ...
    try {
      Log.d(TAG, "Creating LLM Inference with backend: $preferredBackend")
      ...
      Log.e(TAG, "Failed to initialize model '${model.name}': ${e.message}", e)
      
      // Check if it's a TFLite model initialization error
      val errorMessage = e.message ?: "Unknown error"
      if (errorMessage.contains("tflite") || errorMessage.contains("model") || 
          errorMessage.contains("RET_CHECK") || errorMessage.contains("building")) {
        onDone("Model '${model.name}' is not compatible with this device. Try a different model or check device requirements.")
      } else {
  ...
  fun cleanUp(model: Model) {
    ...
    if (instance == null) {
      Log.e(TAG, "Model instance is not LlmModelInstance, cannot clean up")
      model.instance = null
      return
    }
  ...
  fun runInference(
    ...
    val instance = model.instance as? LlmModelInstance
    if (instance == null) {
      Log.e(TAG, "Model instance is null or not LlmModelInstance")
      return
    }
